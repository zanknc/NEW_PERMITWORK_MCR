...
Public Class SEMIStopLotCancel
    Inherits PageBase
    Dim strOPID As String
    Dim actForm As Form
    Dim objCtrSEMI As CtrSemiStopLot
...
#Region "EVENT"
    ...
    Private Sub SEMIStopLotCancel_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Activated
        Try
    ...
    Private Sub SEMIStopLotCancel_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        Try
    ...
    Private Sub SEMIStopLotCancel_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        Try
    ...
    Private Sub SEMIStopLotCancel_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyUp
        Try
    ...
    Private Sub SEMIStopLotCancel_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            GetDefault()
            ResetAll()
        Catch ex As Exception
...
#Region "PRIVATE FUNCTION"
    ...
    Private Function CheckStopLotCancel() As Integer
        Try
            ...
            objCtrSEMI = New CtrSemiStopLot(MyBase.objDBBase.Conn)
            ...
            Try
                ...
                '*************************************************
                cntRec = objCtrSEMI.DeleteSEMIStopLot(objTran)
                If cntRec < 0 Then 'RETURN ERROR
                    MyBase.IsErrMsg = True
                    MyBase.Message = Me.GetPurposeVal("MESG", "7103")
                    Exit Function
                Else

                    Dim LotNo As String
                    Dim LotNoSub As String
                    For i As Integer = 0 To gLot.Rows.Count - 1
                        LotNo = gLot.Rows(i).Cells("LotNo").Value
                        LotNoSub = gLot.Rows(i).Cells("LotNoSub").Value
                        '***********************************************
                        '2.Insert data to  temporary (SEMIStopLot table)
                        '***********************************************
                        cntRec = objCtrSEMI.InsertSEMIStopLot(LotNo, LotNoSub, objTran)
                        If cntRec <= 0 Then 'RETURN RECORD INSERT
                            objTran.Rollback()
                            MyBase.IsErrMsg = True
                            MyBase.Message = Me.GetPurposeVal("MESG", "7104")
                            Exit Function
                        End If
                    Next
                End If
                cntRec = objCtrSEMI.CheckSEMIStopLotCancel(objTran)
                objTran.Commit()

                Dim dtTemp As New DataTable
                dtTemp = objCtrSEMI.GetChecktData()
    ...
    Private Sub GetDefault()
        Try
            Me.Title = Me.GetPurposeVal("TITL", "7006")
    ...
    Private Function RunStopLotCancel() As Integer
        Try
            ...
            Try
                cntRec = objCtrSEMI.RunSEMIStopLotCancel(objTran)
                If cntRec <= 0 Then 'RETURN RECORD UPDATE
                    objTran.Rollback()
                    MyBase.IsErrMsg = True
                    MyBase.Message = Me.GetPurposeVal("MESG", "7107")
                Else
                    objTran.Commit()
                    ...
                    Finally
                        ...
                        MyBase.Message = Me.GetPurposeVal("MESG", "7108")
                    End Try
    ...
    Public Function RunStopLotStatusLot(ByVal OPID As String, ByVal StatusCode As String, ByVal Flag As Integer) As Integer
        Try
            ...
            cntRec = objCtrMainten.UpdateStatusLotSemi(OPID, StatusCode, 1)
