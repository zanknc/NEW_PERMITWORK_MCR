...
Partial Class Confinde_Apporve
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Using con As New SqlConnection(constr)
            Using cmd As New SqlCommand("select * from SAFETY_CONFINDE WHERE SCNO =  '" & lb_opno.Text & "'")
    ...
    Private Sub CHECKSTAMP()
        ...
        cmd.CommandText = "SAFETY_CONFINDE_SELECT"
    ...
    Private Sub CHECKIPAPPROVE()
        Dim ConStr_SAFETY As String = ConfigurationManager.ConnectionStrings("CONN_SAFETY").ConnectionString
        Dim con As New SqlConnection(ConStr_SAFETY)
        Dim strQuery As String = "SELECT * FROM  SAFETY_CERTIFICATE WHERE StatusCer  =  '" & "Cerlicensor" & "'"
        '_ & " AREADEP = @AREADEP"
        Dim cmd As New SqlCommand()
    ...
    Protected Sub btnapp_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnapp.Click
        ...
        If CER_APPNAME = "WAITING APPROVE" And PERMIT = "Cerlicensor" Then
            ...
            Dim ConStr_SAFETY As String = ConfigurationManager.ConnectionStrings("CONN_SAFETY").ConnectionString
            Dim con As New SqlConnection(ConStr_SAFETY)
            Dim cmd As New SqlCommand("UPDATE SAFETY_CONFINDE SET SCTITLEAPPNAME =@SCTITLEAPPNAME , SCAPPNAME = @SCAPPNAME, SCEMAILAPPNAME = @SCEMAILAPPNAME, " _
            ...
            Response.Write("<Script>")
            Response.Write("alert('!!! CONFINDE SPACE  STAMP OK   !!!');")
            Response.Write("location.href='P_THE_CONFINED.aspx';")
            Response.Write("</Script>")
            Exit Sub
        End If
        ...
        If CER_SAFETYNAME = "WAITING APPROVE" And PERMIT = "CerSAFETY" Then
            ...
            Dim ConStr_SAFETY As String = ConfigurationManager.ConnectionStrings("CONN_SAFETY").ConnectionString
            Dim con As New SqlConnection(ConStr_SAFETY)
            Dim cmd As New SqlCommand("UPDATE SAFETY_CONFINDE SET SCTITLESAFETYNAME = @SCTITLESAFETYNAME ,  SCSAFETYNAME = @SCSAFETYNAME, SCSAFETYEMAILNAME = @SCSAFETYEMAILNAME, " _
